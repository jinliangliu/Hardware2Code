...
| `adc_uart` | ADC + USART 采样通信 | ADC, USART, GPIO |
...
### 2. adc_uart
- **功能**：ADC 采样通过 USART 输出。
- **硬件连接**：ADC 输入 → 对应引脚，USART → PA2(TX)/PA3(RX)。
- **生成命令**：`python generator/generate.py -i examples/adc_uart/hardware.yaml -o output/adc_uart`
...
- **硬件连接**：LED → PC0，按键 → PC13。
- **生成命令**：`python generator/generate.py -i examples/rtc_advanced/hardware.yaml -o output/rtc_advanced`
...
- **生成命令**：`python generator/generate.py -i examples/i2c_spi_demo/hardware.yaml -o output/i2c_spi_demo`

### 8. low_power_demo
- **功能**：STOP0 低功耗模式演示，通过 RTC 和按键 EXTI 唤醒。
- **硬件连接**：LED → PA5，按键 → PC13（EXTI 下降沿唤醒）。
- **生成命令**：`python generator/generate.py -i examples/low_power_demo/hardware.yaml -o output/low_power_demo`
...
- **硬件连接**：LED → PC0，按键 → PC13。
- **生成命令**：`python generator/generate.py -i examples/substate_demo/hardware.yaml -o output/substate_demo`

### 10. parallel_states
- **功能**：并行区域（parallel regions）演示，多个独立状态机同时运行。
- **硬件连接**：LED → PC0，按键 → PC13。
- **生成命令**：`python generator/generate.py -i examples/parallel_states/hardware.yaml -o output/parallel_states`

### 11. parallel_comm
- **功能**：并行区域之间的跨区域通信（send_to）。
- **硬件连接**：LED → PC0，按键 → PC13。
- **生成命令**：`python generator/generate.py -i examples/parallel_comm/hardware.yaml -o output/parallel_comm`

### 12. ref_demo
- **功能**：状态机 ref 引用基础用法，复用子状态机定义。
- **硬件连接**：LED → PC0，按键 → PC13。
- **生成命令**：`python generator/generate.py -i examples/ref_demo/hardware.yaml -o output/ref_demo`

### 13. nested_ref
- **功能**：ref 引用 + namespace 命名空间隔离，支持多层嵌套状态机复用。
- **硬件连接**：LED → PC0，按键 → PC13。
- **生成命令**：`python generator/generate.py -i examples/nested_ref/hardware.yaml -o output/nested_ref`

### 14. timeline_demo
- **功能**：timeline 时间序列动作，按时间轴依次执行多个延迟动作。
- **硬件连接**：LED → PC0，按键 → PC13。
- **生成命令**：`python generator/generate.py -i examples/timeline_demo/hardware.yaml -o output/timeline_demo`
...
- **硬件连接**：LED → PC0，按键 → PC13。
- **生成命令**：`python generator/generate.py -i examples/bootloader_demo/hardware.yaml -o output/bootloader_demo`

### 16. fota_demo
- **功能**：BSDIFF 差分固件升级基础框架，含 Bootloader + CLI 触发 + 双分区 A/B 镜像管理。
- **硬件连接**：LED → PA5；USART2 → PA2(TX)/PA3(RX)。
- **生成命令**：`python generator/generate.py -i examples/fota_demo/hardware.yaml -o output/fota_demo`

### 17. fota_flow
- **功能**：FOTA 升级完整状态机（空闲/等待补丁/校验/应用/重试/成功/错误 7 状态），含补丁超时和校验失败重试。
- **硬件连接**：LED → PC0，按键 → PC13。
- **生成命令**：`python generator/generate.py -i examples/fota_flow/hardware.yaml -o output/fota_flow`
...
- **生成命令**：`python generator/generate.py -i examples/cli_demo/hardware.yaml -o output/cli_demo`

### 19. modbus_demo
- **功能**：Modbus RTU 从机通信，基于 RS485 物理层。
- **硬件连接**：USART1 → PB6(TX)/PB7(RX) (9600bps)；RS485 DE → PA1；LED → PC0；按键 → PC13。
- **生成命令**：`python generator/generate.py -i examples/modbus_demo/hardware.yaml -o output/modbus_demo`

### 20. cellular_mqtt
- **功能**：4G Cat.1 蜂窝网络拨号 + MQTT 上云，含三态业务状态机（断连/连接中/已连接）。
- **硬件连接**：USART2 → PA2(TX)/PA3(RX) (115200bps)；LED → PC0；按键 → PC13。
- **生成命令**：`python generator/generate.py -i examples/cellular_mqtt/hardware.yaml -o output/cellular_mqtt`

### 21. cellular_test
- **功能**：最精简的 4G Cat.1 通信基础测试，仅验证 AT 指令交互和 MQTT 连接能力。
- **硬件连接**：USART2 → PA2(TX)/PA3(RX) (115200bps)；LED → PC0。
- **生成命令**：`python generator/generate.py -i examples/cellular_test/hardware.yaml -o output/cellular_test`
