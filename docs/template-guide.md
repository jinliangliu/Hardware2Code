...
通过 `business_flow` 节点定义状态机，自动生成 `statemachine.c/h`，并在事件管理器中集成。
...
支持的动作：
 toggle_led – 翻转 LED（需要 led_task 存在）
 start_timer <name> <period_ms> – 启动一次性软件定时器，到期产生 EVENT_TIMER_EXPIRED_<name> 事件
 stop_timer <name> – 停止指定定时器

可用事件：
所有在 event_mgr.h 中定义的事件（如 EVENT_BUTTON_PRESS, EVENT_RTC_TICK）以及动态生成的 EVENT_TIMER_EXPIRED_<name>。
