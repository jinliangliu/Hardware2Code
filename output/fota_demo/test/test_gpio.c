...
void test_PC14_configuration(void) {
    MX_GPIO_Init();
    /* 获取该引脚对应索引的配置（按YAML顺序） */
    uint32_t idx = 3;
    uint32_t expected_pin = GPIO_PIN_14;
    uint32_t actual_pin = mock_HAL_GPIO_Init_get_pin(idx);
    TEST_ASSERT_EQUAL_HEX32(expected_pin, actual_pin);

    TEST_ASSERT_EQUAL_UINT32(GPIO_MODE_IT_FALLING, mock_HAL_GPIO_Init_get_mode(idx));
}
void test_PC15_configuration(void) {
    MX_GPIO_Init();
    /* 获取该引脚对应索引的配置（按YAML顺序） */
    uint32_t idx = 4;
    uint32_t expected_pin = GPIO_PIN_15;
    uint32_t actual_pin = mock_HAL_GPIO_Init_get_pin(idx);
    TEST_ASSERT_EQUAL_HEX32(expected_pin, actual_pin);
...
int main(void) {
    ...
    RUN_TEST(test_PC14_configuration);
    RUN_TEST(test_PC15_configuration);
    return UNITY_END();
}