...
UART_HandleTypeDef huart_debug_uart;

/* ------- Task handles ------- */
TaskHandle_t led_task_handle = NULL;
TaskHandle_t fota_monitor_handle = NULL;

/* ------- Task prototypes ------- */
void led_task(void *pvParameters);
void fota_monitor(void *pvParameters);
...
static void MX_USART2_UART_Init(void)
{
    ...
    huart_debug_uart.Instance = USART2;
    huart_debug_uart.Init.BaudRate = 115200;
    huart_debug_uart.Init.WordLength = UART_WORDLENGTH_8B;
    huart_debug_uart.Init.StopBits = UART_STOPBITS_1;
    huart_debug_uart.Init.Parity = UART_PARITY_NONE;
    huart_debug_uart.Init.Mode = UART_MODE_TX_RX;
    huart_debug_uart.Init.HwFlowCtl = UART_HWCONTROL_NONE;
    huart_debug_uart.Init.OverSampling = UART_OVERSAMPLING_16;
    if (HAL_UART_Init(&huart_debug_uart) != HAL_OK) while(1);
}
...
int main(void)
{
    ...
    cli_init(&huart_debug_uart);

    RTC_Start();

    /* Create application tasks (user-defined) */
    xTaskCreate( led_task, "led_task", 128, NULL, 1, &led_task_handle );
    xTaskCreate( fota_monitor, "fota_monitor", 512, NULL, 2, &fota_monitor_handle );
