...
static rtc_timer_handle_t RUNNING_timeout_handle = NULL;
static void RUNNING_timeout_cb(void *arg) {
    event_t evt = { .id = EVENT_TIMER_EXPIRED_RUNNING_timeout, .param = 0 };
    statemachine_process(&evt);
}
...
void statemachine_process(event_t *evt) {
    ...
    case STATE_IDLE:
        if (
            ...
            /* 执行目标状态的进入动作并启动时间序列 */
            {
                ...
                rtc_timer_handle_t h = RTC_TimerCreate(period, RTC_TIMER_MODE_ONE_SHOT,
                                                       RUNNING_timeout_cb, NULL);
                RUNNING_timeout_handle = h;
                RTC_TimerStart(h);
            }
        }
        else if (
    ...
    case STATE_RUNNING:
        if (
            ...
            /* 停止并删除当前状态的超时定时器 */
            if (RUNNING_timeout_handle) {
                RTC_TimerStop(RUNNING_timeout_handle);
                RTC_TimerDelete(RUNNING_timeout_handle);
                RUNNING_timeout_handle = NULL;
            }
            /* 停止并删除所有 defer 定时器 */
            extern void led_task_notify(void);
            led_task_notify();
            ...
            (evt->id == EVENT_TIMER_EXPIRED_RUNNING_timeout)
        ) {
            /* 停止并删除当前状态的超时定时器 */
            if (RUNNING_timeout_handle) {
                RTC_TimerStop(RUNNING_timeout_handle);
