...
void test_IDLE_to_RUNNING_on_BUTTON_PRESS(void) {
    event_t evt = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&evt);
    TEST_ASSERT_EQUAL(1, led_notify_count);
}
void test_IDLE_to_IDLE_on_TIMER_EXPIRED_IDLE_timeout(void) {
    event_t evt = { .id = EVENT_TIMER_EXPIRED_IDLE_timeout };
    statemachine_process(&evt);
    TEST_ASSERT_EQUAL(0, led_notify_count);
}

void test_RUNNING_to_IDLE_on_TIMER_EXPIRED_RUNNING_timeout(void) {
    event_t evt_fwd = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&evt_fwd);
    led_notify_count = 0;
    event_t evt_back = { .id = EVENT_TIMER_EXPIRED_RUNNING_timeout };
...
void test_IDLE_to_IDLE_on_TIMER_EXPIRED_IDLE_timeout(void) {
    ...
    statemachine_process(&evt_fwd);
    led_notify_count = 0;
    event_t evt_back = { .id = EVENT_TIMER_EXPIRED_IDLE_timeout };
    statemachine_process(&evt_back);
    TEST_ASSERT_EQUAL(0, led_notify_count);
}
void test_IDLE_to_IDLE_on_TIMER_EXPIRED_IDLE_timeout(void) {
...
void test_IDLE_to_IDLE_on_TIMER_EXPIRED_IDLE_timeout(void) {
    event_t evt_fwd = { .id = EVENT_TIMER_EXPIRED_RUNNING_timeout };
    statemachine_process(&evt_fwd);
    led_notify_count = 0;
    event_t evt_back = { .id = EVENT_TIMER_EXPIRED_IDLE_timeout };
    statemachine_process(&evt_back);
...
int main(void) {
    ...
    RUN_TEST(test_IDLE_to_RUNNING_on_BUTTON_PRESS);
    RUN_TEST(test_IDLE_to_IDLE_on_TIMER_EXPIRED_IDLE_timeout);
    RUN_TEST(test_RUNNING_to_IDLE_on_BUTTON_PRESS);
    RUN_TEST(test_RUNNING_to_IDLE_on_TIMER_EXPIRED_RUNNING_timeout);
    RUN_TEST(test_RUNNING_to_IDLE_on_TIMER_EXPIRED_RUNNING_timeout);
    RUN_TEST(test_IDLE_to_IDLE_on_TIMER_EXPIRED_IDLE_timeout);
    RUN_TEST(test_IDLE_to_IDLE_on_TIMER_EXPIRED_IDLE_timeout);
    RUN_TEST(test_IDLE_to_IDLE_on_TIMER_EXPIRED_IDLE_timeout);
