...
UART_HandleTypeDef huart_uart1;
...
static void MX_USART1_UART_Init(void)
{
    ...
    huart_uart1.Instance = USART1;
    huart_uart1.Init.BaudRate = 9600;
    huart_uart1.Init.WordLength = UART_WORDLENGTH_8B;
    huart_uart1.Init.StopBits = UART_STOPBITS_1;
    huart_uart1.Init.Parity = UART_PARITY_NONE;
    huart_uart1.Init.Mode = UART_MODE_TX_RX;
    huart_uart1.Init.HwFlowCtl = UART_HWCONTROL_NONE;
    huart_uart1.Init.OverSampling = UART_OVERSAMPLING_16;
    if (HAL_UART_Init(&huart_uart1) != HAL_OK) while(1);
}
...
int main(void)
{
    ...
    xTaskCreate( modbus_task, "modbus_task", 512, NULL, 3, &modbus_task_handle );
