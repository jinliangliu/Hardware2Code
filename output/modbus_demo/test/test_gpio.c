...
void test_PA9_configuration(void) {
    MX_GPIO_Init();
    /* 获取该引脚对应索引的配置（按YAML顺序） */
    uint32_t idx = 2;
    uint32_t expected_pin = GPIO_PIN_9;
    uint32_t actual_pin = mock_HAL_GPIO_Init_get_pin(idx);
    TEST_ASSERT_EQUAL_HEX32(expected_pin, actual_pin);

}
void test_PA10_configuration(void) {
    MX_GPIO_Init();
    /* 获取该引脚对应索引的配置（按YAML顺序） */
    uint32_t idx = 3;
    uint32_t expected_pin = GPIO_PIN_10;
    uint32_t actual_pin = mock_HAL_GPIO_Init_get_pin(idx);
    TEST_ASSERT_EQUAL_HEX32(expected_pin, actual_pin);

}
void test_PA1_configuration(void) {
    MX_GPIO_Init();
    /* 获取该引脚对应索引的配置（按YAML顺序） */
    uint32_t idx = 4;
    uint32_t expected_pin = GPIO_PIN_1;
    uint32_t actual_pin = mock_HAL_GPIO_Init_get_pin(idx);
    TEST_ASSERT_EQUAL_HEX32(expected_pin, actual_pin);
...
int main(void) {
    ...
    RUN_TEST(test_PA9_configuration);
    RUN_TEST(test_PA10_configuration);
    RUN_TEST(test_PA1_configuration);
    return UNITY_END();
}