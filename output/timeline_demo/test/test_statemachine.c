...
void test_IDLE_to_PLAYING_on_BUTTON_PRESS(void) {
    event_t evt = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&evt);
    TEST_ASSERT_EQUAL(1, led_notify_count);
}

void test_PLAYING_to_IDLE_on_TIMER_EXPIRED_PLAYING_timeout(void) {
    event_t evt_fwd = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&evt_fwd);
    led_notify_count = 0;
    event_t evt_back = { .id = EVENT_TIMER_EXPIRED_PLAYING_timeout };
...
int main(void) {
    ...
    RUN_TEST(test_IDLE_to_PLAYING_on_BUTTON_PRESS);
    RUN_TEST(test_PLAYING_to_IDLE_on_TIMER_EXPIRED_PLAYING_timeout);
    RUN_TEST(test_PLAYING_to_IDLE_on_TIMER_EXPIRED_PLAYING_timeout);
