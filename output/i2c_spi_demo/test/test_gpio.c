...
void test_MX_GPIO_Init_should_call_HAL_for_all_pins(void) {
    MX_GPIO_Init();
    TEST_ASSERT_EQUAL_UINT32(8, mock_HAL_GPIO_Init_get_count());
}
...
void test_PA5_configuration(void) {
    MX_GPIO_Init();
    /* 获取该引脚对应索引的配置（按YAML顺序） */
    uint32_t idx = 2;
    uint32_t expected_pin = GPIO_PIN_5;
    uint32_t actual_pin = mock_HAL_GPIO_Init_get_pin(idx);
    TEST_ASSERT_EQUAL_HEX32(expected_pin, actual_pin);

}
void test_PA6_configuration(void) {
    MX_GPIO_Init();
    /* 获取该引脚对应索引的配置（按YAML顺序） */
    uint32_t idx = 3;
    uint32_t expected_pin = GPIO_PIN_6;
...
void test_PA7_configuration(void) {
    MX_GPIO_Init();
    /* 获取该引脚对应索引的配置（按YAML顺序） */
    uint32_t idx = 4;
    uint32_t expected_pin = GPIO_PIN_7;
...
void test_PA4_configuration(void) {
    MX_GPIO_Init();
    /* 获取该引脚对应索引的配置（按YAML顺序） */
    uint32_t idx = 5;
    uint32_t expected_pin = GPIO_PIN_4;
    uint32_t actual_pin = mock_HAL_GPIO_Init_get_pin(idx);
    TEST_ASSERT_EQUAL_HEX32(expected_pin, actual_pin);

    TEST_ASSERT_EQUAL_UINT32(GPIO_MODE_OUTPUT_PP, mock_HAL_GPIO_Init_get_mode(idx));
}
void test_PA0_configuration(void) {
    MX_GPIO_Init();
    /* 获取该引脚对应索引的配置（按YAML顺序） */
    uint32_t idx = 6;
...
int main(void) {
    ...
    RUN_TEST(test_PA5_configuration);
    RUN_TEST(test_PA6_configuration);
    RUN_TEST(test_PA7_configuration);
    RUN_TEST(test_PA4_configuration);
    RUN_TEST(test_PA0_configuration);
