...
void test_idle_to_reading_on_BUTTON_PRESS(void) {
    event_t evt = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&evt);
    TEST_ASSERT_EQUAL(1, led_notify_count);
}

void test_reading_to_idle_on_BUTTON_PRESS(void) {
...
void test_storing_to_idle_on_STORE_DONE(void) {
    event_t evt_fwd = { .id = EVENT_SENSOR_READY };
    statemachine_process(&evt_fwd);
    led_notify_count = 0;
    event_t evt_back = { .id = EVENT_STORE_DONE };
    statemachine_process(&evt_back);
    TEST_ASSERT_EQUAL(1, led_notify_count);
}
...
int main(void) {
    ...
    RUN_TEST(test_idle_to_reading_on_BUTTON_PRESS);
    RUN_TEST(test_reading_to_storing_on_SENSOR_READY);
    RUN_TEST(test_reading_to_idle_on_BUTTON_PRESS);
    RUN_TEST(test_storing_to_idle_on_STORE_DONE);
    RUN_TEST(test_reading_to_idle_on_BUTTON_PRESS);
    RUN_TEST(test_storing_to_idle_on_STORE_DONE);
