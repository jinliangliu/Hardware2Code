...
void HAL_RTC_MspInit(RTC_HandleTypeDef* hrtc)
{
    if (hrtc->Instance == RTC) {
        ...
        /* 尝试使用外部低速晶振 LSE (32.768kHz)，失败则回退到 LSI */
...
void RTC_Init(void)
{
    ...
    /* Explicitly clear stale RTC wakeup/alarm flags from previous runs
     * (RTC backup domain survives system reset). Must be done BEFORE
     * enabling the NVIC to prevent spurious ISR firing that could crash
     * FreeRTOS before the scheduler starts. */
...
/**
 * @brief  Restore periodic RTC wakeup configuration after tickless idle.
 *         vPortSuppressTicksAndSleep() reconfigures the wakeup timer to
 *         CK_SPRE (1 Hz) for STOP mode; this function restores the original
 *         RTCCLK_DIV16 (2048 Hz) 100ms periodic tick so software timers
 *         resume firing after the system wakes up.
 */
