...
void test_IDLE_to_ACTIVE_on_BUTTON_PRESS(void) {
    event_t evt = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&evt);
    TEST_ASSERT_EQUAL(1, led_notify_count);
}
void test_IDLE_to_RESET_on_BUTTON_PRESS(void) {
    event_t evt = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&evt);
    TEST_ASSERT_EQUAL(1, led_notify_count);
}
void test_IDLE_to_TIMEOUT_on_TIMER_EXPIRED_IDLE_timeout(void) {
    event_t evt = { .id = EVENT_TIMER_EXPIRED_IDLE_timeout };
    statemachine_process(&evt);
    TEST_ASSERT_EQUAL(1, led_notify_count);
}

void test_ACTIVE_to_IDLE_on_TIMER_EXPIRED_exit_timer(void) {
    event_t evt_fwd = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&evt_fwd);
    led_notify_count = 0;
    event_t evt_back = { .id = EVENT_TIMER_EXPIRED_exit_timer };
    statemachine_process(&evt_back);
    TEST_ASSERT_EQUAL(1, led_notify_count);
}
void test_RESET_to_IDLE_on_RTC_TICK(void) {
    event_t evt_fwd = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&evt_fwd);
    led_notify_count = 0;
    event_t evt_back = { .id = EVENT_RTC_TICK };
    statemachine_process(&evt_back);
    TEST_ASSERT_EQUAL(1, led_notify_count);
}
void test_TIMEOUT_to_IDLE_on_RTC_TICK(void) {
    event_t evt_fwd = { .id = EVENT_TIMER_EXPIRED_IDLE_timeout };
    statemachine_process(&evt_fwd);
    led_notify_count = 0;
    event_t evt_back = { .id = EVENT_RTC_TICK };
    statemachine_process(&evt_back);
...
int main(void) {
    ...
    RUN_TEST(test_IDLE_to_ACTIVE_on_BUTTON_PRESS);
    RUN_TEST(test_IDLE_to_RESET_on_BUTTON_PRESS);
    RUN_TEST(test_IDLE_to_TIMEOUT_on_TIMER_EXPIRED_IDLE_timeout);
    RUN_TEST(test_ACTIVE_to_IDLE_on_RTC_TICK);
    RUN_TEST(test_ACTIVE_to_IDLE_on_TIMER_EXPIRED_exit_timer);
    RUN_TEST(test_RESET_to_IDLE_on_RTC_TICK);
    RUN_TEST(test_TIMEOUT_to_IDLE_on_RTC_TICK);
    RUN_TEST(test_ACTIVE_to_IDLE_on_TIMER_EXPIRED_exit_timer);
    RUN_TEST(test_RESET_to_IDLE_on_RTC_TICK);
    RUN_TEST(test_TIMEOUT_to_IDLE_on_RTC_TICK);
