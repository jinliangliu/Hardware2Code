...
/* Test: BUTTON_PRESS enters PROCESS substate, auto-enters first sub-state */
void test_enter_substate_initial_step(void) {
    event_t evt = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&evt);
    /* IDLE→PROCESS transition: toggle_led fired once */
...
void test_substate_step1_to_step2(void) {
    ...
    /* Send STEP_NEXT (what defer would publish after 1000ms) */
    event_t step_evt = { .id = EVENT_STEP_NEXT };
...
void test_substate_return_to_idle(void) {
    /* Navigate: IDLE → PROCESS → step 1 → step 2 */
    event_t press = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&press);
    event_t evt1 = { .id = EVENT_STEP_NEXT };
    statemachine_process(&evt1);
    /* Send STEP_RETURN → return → PROCESS transition → IDLE */
    event_t evt2 = { .id = EVENT_STEP_RETURN };
