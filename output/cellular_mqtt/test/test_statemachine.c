...
void test_DISCONNECTED_to_CONNECTING_on_BUTTON_PRESS(void) {
    event_t evt = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&evt);
    TEST_ASSERT_EQUAL(2, led_notify_count);
}

void test_CONNECTED_to_DISCONNECTED_on_BUTTON_PRESS(void) {
    event_t evt_fwd = { .id = EVENT_TIMER_EXPIRED_CONNECTING_timeout };
...
void test_roundtrip_cycle(void) {
    ...
    TEST_ASSERT_EQUAL(12, led_notify_count);
}

int main(void) {
    UNITY_BEGIN();
    RUN_TEST(test_initial_state_actions);
    RUN_TEST(test_DISCONNECTED_to_CONNECTING_on_BUTTON_PRESS);
    RUN_TEST(test_CONNECTING_to_CONNECTED_on_TIMER_EXPIRED_CONNECTING_timeout);
    RUN_TEST(test_CONNECTED_to_DISCONNECTED_on_BUTTON_PRESS);
    RUN_TEST(test_CONNECTED_to_DISCONNECTED_on_BUTTON_PRESS);
