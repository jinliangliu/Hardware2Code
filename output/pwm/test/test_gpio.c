...
void test_MX_GPIO_Init_should_call_HAL_for_all_pins(void) {
    MX_GPIO_Init();
    TEST_ASSERT_EQUAL_UINT32(3, mock_HAL_GPIO_Init_get_count());
}

void test_PA8_configuration(void) {
    MX_GPIO_Init();
    /* 获取该引脚对应索引的配置（按YAML顺序） */
    uint32_t idx = 0;
    uint32_t expected_pin = GPIO_PIN_8;
...
int main(void) {
    ...
    RUN_TEST(test_PA8_configuration);
