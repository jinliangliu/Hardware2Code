...
void test_MAIN_IDLE_to_DEEP_on_BUTTON_PRESS(void) {
    event_t evt = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&evt);
    TEST_ASSERT_EQUAL(0, led_notify_count);
}

void test_DEEP_to_MAIN_IDLE_on_HIGH_COUNT(void) {
    event_t evt_fwd = { .id = EVENT_BUTTON_PRESS };
    statemachine_process(&evt_fwd);
    led_notify_count = 0;
    event_t evt_back = { .id = EVENT_HIGH_COUNT };
...
int main(void) {
    ...
    RUN_TEST(test_MAIN_IDLE_to_DEEP_on_BUTTON_PRESS);
    RUN_TEST(test_DEEP_to_MAIN_IDLE_on_HIGH_COUNT);
    RUN_TEST(test_DEEP_to_MAIN_IDLE_on_HIGH_COUNT);
